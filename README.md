...
This repository provides the analysis workflow and supplementary materials for a machine-learning framework that prioritizes nanoparticle formulations with high predicted tumor delivery efficiency.
...
The virtual screening workflow generated 50,000 virtual nanoparticle candidates using chemistry-aware categorical feasibility filtering, real-database Type-MAT-Shape combination constraints, numeric out-of-distribution control, CatBoost-based high-delivery probability prediction, and candidate ranking.
...
The notebook was cleaned for public release by removing temporary debugging code, redundant exploratory analyses, deprecated experiments, and local environment-dependent paths while preserving the reported experimental workflow and results.

The notebook uses repository-relative paths and may require users to adjust input/output directories according to their local execution environment.

Running the notebook may require the datasets and supplementary files provided in this repository.
...
Excel file containing the analytical dataset, data partitioning, high-delivery label definition, endpoint description, threshold sensitivity analysis, variable dictionary, variable ranges, and categorical levels.
...
- 50,000 scored virtual candidates
...
The dashboard presents model-based prioritization results and is intended for exploration of computational predictions. It does not represent experimentally validated nanoparticle formulations.
